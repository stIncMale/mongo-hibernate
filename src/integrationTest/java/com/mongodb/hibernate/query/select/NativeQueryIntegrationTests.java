...
/**
 * - Only DQL is supported. - DQL is always collection-level aggregation. - Projection must be present and specify all
 * columns. - In some situations the type of the persistent attribute has to be specified, e.g., via
 * SqlResultSetMapping.columns. - SqlResultSetMapping.classes is not implemented correctly in Hibernate ORM - it does
                ...
                assertEq(
                        ...
                        session.createNativeQuery(
                                ...
                                .setTupleTransformer((tuple, aliases) -> {
                                    ...
                                    var result = new EmbeddableIntegrationTests.ArraysAndCollections(
                                            (byte[]) tuple[0],
                                            (char[]) tuple[1],
                                            (int[]) tuple[2],
                                            (long[]) tuple[3],
